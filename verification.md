...
- 2026-06-02：发布 `0.42.0` 可安装测试包，收口 InnerBrain 本机失败评估意图混淆汇总；`/inner-brain-eval-local-report [文件名]` 导出的 `word/inner-brain-evaluation-report.md` 会按失败样本的 `expected_intent -> actual_intent` 统计数量并列出典型样本，只读导出且不写入 runtime 训练样本；全量 `unittest` 552 项通过，安装包生成到 `E:\oyzj\ai\jarvis-lite-dist\JarvisLiteSetup-0.42.0.exe`。
