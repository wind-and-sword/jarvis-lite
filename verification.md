...
- 2026-06-03：发布 `0.98.0` 可安装测试包，收口 InnerBrain 本机报告指定文件样本计数提示；`/inner-brain-eval-local-report [文件名]` 的成功反馈会显示 `当前文件样本：N`，指定文件无样本时明确显示 `当前文件样本：0`；全量 `unittest` 568 项通过，安装包生成到 `E:\oyzj\ai\jarvis-lite-dist\JarvisLiteSetup-0.98.0.exe`。
