...
- 2026-06-03：发布 `0.125.0` 可安装测试包，完成任务执行结果摘要采集第一阶段；当前任务运行中执行显式命令后，最近匹配任务事件会写回紧凑结果摘要，`/task-fail` 和 `/task-status` 的失败复盘可展示命令与结果；当前阶段不自动重放外部动作、不点击、不输入；全量 `unittest` 727 项通过，安装包生成到 `E:\oyzj\ai\jarvis-lite-dist\JarvisLiteSetup-0.125.0.exe`。
- 2026-06-03：发布 `0.124.0` 可安装测试包，完成任务状态自动采集第一阶段；当前任务运行中执行普通命令会自动记录最近任务事件，`/task-status` 展示输入原话、路由类型、命令详情、摘要和依据，`/task-fail` 与 `/task-fail-capture` 会把自动采集上下文写入失败复盘；当前阶段不自动重放外部动作、不点击、不输入；全量 `unittest` 726 项通过，安装包生成到 `E:\oyzj\ai\jarvis-lite-dist\JarvisLiteSetup-0.124.0.exe`。
...
- 2026-06-03：发布 `0.92.0` 可安装测试包，收口 InnerBrain 本机全量反馈按文件聚焦待处理失败标签；`/inner-brain-eval-local-failed` 与 `/inner-brain-eval-local-report` 未指定文件时会提示 `按文件聚焦待处理失败：/inner-brain-eval-local-file-failed 文件名`；全量 `unittest` 566 项通过，安装包生成到 `E:\oyzj\ai\jarvis-lite-dist\JarvisLiteSetup-0.92.0.exe`。
- 2026-06-03：发布 `0.91.0` 可安装测试包，收口 InnerBrain 本机报告导出反馈待处理失败计数标签；`/inner-brain-eval-local-report [文件名]` 的成功反馈会把导出计数显示为 `待处理失败样本：N`；全量 `unittest` 566 项通过，安装包生成到 `E:\oyzj\ai\jarvis-lite-dist\JarvisLiteSetup-0.91.0.exe`。
- 2026-06-03：发布 `0.90.0` 可安装测试包，收口 InnerBrain 本机失败帮助待处理标签；`/help` 和本机失败视图运行日志统一使用“待处理失败样本/待处理失败报告”语义；全量 `unittest` 566 项通过，安装包生成到 `E:\oyzj\ai\jarvis-lite-dist\JarvisLiteSetup-0.90.0.exe`。
- 2026-06-03：发布 `0.89.0` 可安装测试包，收口 InnerBrain 本机报告导出待处理失败标题；`/inner-brain-eval-local-report [文件名]` 的导出反馈、运行日志和 Markdown H1 标题统一为“本机评估待处理失败报告”；全量 `unittest` 566 项通过，安装包生成到 `E:\oyzj\ai\jarvis-lite-dist\JarvisLiteSetup-0.89.0.exe`。
- 2026-06-03：发布 `0.88.0` 可安装测试包，收口 InnerBrain 本机文件候选待处理报告标签；`/inner-brain-eval-local`、`/inner-brain-eval-local-failed` 和 `/inner-brain-eval-local-resolved` 的文件候选在同文件仍有失败时会用 `待处理报告：/inner-brain-eval-local-report 文件名` 提示按文件导出待处理失败报告；全量 `unittest` 566 项通过，安装包生成到 `E:\oyzj\ai\jarvis-lite-dist\JarvisLiteSetup-0.88.0.exe`。
- 2026-06-03：发布 `0.87.0` 可安装测试包，收口 InnerBrain 本机评估样本保存反馈待处理失败标签；保存本机 evaluation 样本后会用 `只看待处理失败样本：/inner-brain-eval-local-failed` 和 `导出样本文件待处理失败报告：/inner-brain-eval-local-report runtime.jsonl` 提示后续验证；全量 `unittest` 566 项通过，安装包生成到 `E:\oyzj\ai\jarvis-lite-dist\JarvisLiteSetup-0.87.0.exe`。
- 2026-06-03：发布 `0.86.0` 可安装测试包，收口 InnerBrain 本机当前文件待处理失败报告标签；当前文件总览、当前文件失败视图和当前文件已处理视图在仍有失败时会用 `导出当前文件待处理失败报告：/inner-brain-eval-local-report 文件名` 提示导出当前文件待处理失败报告；全量 `unittest` 566 项通过，安装包生成到 `E:\oyzj\ai\jarvis-lite-dist\JarvisLiteSetup-0.86.0.exe`。
- 2026-06-03：发布 `0.85.0` 可安装测试包，收口 InnerBrain 本机失败视图按文件待处理失败报告标签；`/inner-brain-eval-local-failed` 未指定文件时会用 `按文件导出待处理失败报告：/inner-brain-eval-local-report 文件名` 提示按文件导出待处理失败报告；全量 `unittest` 566 项通过，安装包生成到 `E:\oyzj\ai\jarvis-lite-dist\JarvisLiteSetup-0.85.0.exe`。
