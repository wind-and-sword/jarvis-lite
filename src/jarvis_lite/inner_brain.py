...
class InnerBrainEvaluationReport:
    ...
    @property
    def failed_expected_intent_counts(self) -> Mapping[str, int]:
        counts: dict[str, int] = {}
        for case_result in self.failed_case_results:
            expected_intent = case_result.case.expected_intent
            counts[expected_intent] = counts.get(expected_intent, 0) + 1
        return counts
...
def describe_inner_brain_evaluation(report: InnerBrainEvaluationReport, failures_only: bool = False) -> str:
    ...
    if failures_only and report.source_file_filter is None:
        ...
        if failed_source_file_counts:
            ...
            for source_file, count in sorted_failed_source_file_counts:
                lines.append(f"- {source_file}：{count} 条：/inner-brain-eval-local-file-failed {source_file}")
    ...
    if failures_only and report.failed_case_results:
        ...
        expected_intent_examples: dict[str, str] = {}
        for case_result in report.failed_case_results:
            expected_intent_examples.setdefault(case_result.case.expected_intent, case_result.case.text)
        lines.append("失败期望意图汇总：")
    ...
    if report.failed_case_results:
        ...
        file_intent_confusion_fix_suggestions: dict[tuple[str, str], list[str]] = {}
        if report.source_file_filter is None:
            for case_result in report.failed_case_results:
                source_file = case_result.case.source_file
                if source_file is None or case_result.case.expected_intent == case_result.result.intent:
                    continue
                confusion = f"{case_result.case.expected_intent} -> {case_result.result.intent}"
                file_intent_confusion_fix_suggestions.setdefault((source_file, confusion), []).append(
                    _inner_brain_evaluation_fix_suggestion(case_result)
                )
        if file_intent_confusion_fix_suggestions:
            lines.append("失败文件意图混淆修复建议：")
            for (source_file, confusion), suggestions in sorted(
                file_intent_confusion_fix_suggestions.items(),
                key=lambda item: (-len(item[1]), item[0][0], item[0][1]),
            ):
                lines.append(f"- {source_file}：{confusion}：{len(suggestions)} 条")
                for suggestion in suggestions:
                    lines.append(f"  - {suggestion}")
        lines.append("失败修复建议：")
        for case_result in report.failed_case_results:
