...
class JarvisAgent:
    ...
    def handle(self, user_input: str) -> str:
        ...
        if prompt in {
            ...
            "config-candidate-history",
            "/memory-config-candidate-history",
            "memory-config-candidate-history",
        }:
            self.tools.run("record_log", message="查看记忆与配置候选历史")
            return describe_memory_config_candidate_history(self.paths)
        ...
        if prompt in {"/authorization-status", "authorization-status"}:
            ...
            return describe_authorization_status(self.paths)
        ...
        return self._apply_preference_local_answer_note(memory_answer, "memory")
    ...
    def _handle_command(self, prompt: str) -> str:
        ...
        if command == "/config-candidate-restore":
            if not args:
                return "用法：/config-candidate-restore 编号"
            try:
                candidate_index = int(args[0])
            except ValueError:
                return "候选编号必须是数字。"
    ...
    def _help(self) -> str:
        return "\n".join(
            [
                ...
                "/inner-brain-eval-failed：只显示 InnerBrain 固定与本机评估集失败样本",
    ...
    def _answer_from_data(self, question: str) -> str:
        ...
        return self._apply_preference_local_answer_note(answer_from_matches(matches), "knowledge")

    def _apply_preference_local_answer_note(self, answer: str, answer_type: str) -> str:
        note = describe_preference_local_answer_note(self.paths, answer_type)
        if not note:
            return answer
        return f"{answer}\n\n{note}"
