...
class AgentTests(unittest.TestCase):
    ...
    def test_config_candidate_confirm_and_undo_preference(self):
        ...
        self.assertIn("候选已恢复为活跃", undo_response)
        self.assertIn("偏好：0 条", manager_status_after_undo)
        self.assertIn("1. 偏好：回答尽量简洁", list_response)
        self.assertEqual(json.loads(preferences_path.read_text(encoding="utf-8")), {"preferences": []})
        self.assertNotIn("回答尽量简洁", preferences_path.read_text(encoding="utf-8"))

    def test_preference_status_enable_and_disable_commands_manage_saved_preferences(self):
        self.agent.handle("/config-candidate-add preference 回答尽量简洁")
        ...
        missing_disable = self.agent.handle("/preference-disable 2")
        help_text = self.agent.handle("/help")
        status = self.agent.handle("/status")
        preferences_payload = json.loads((self.paths.config_dir / "preferences.local.json").read_text(encoding="utf-8"))
    ...
    def test_preference_preview_command_reports_enabled_preferences_without_applying(self):
        self.agent.handle("/config-candidate-add preference 回答尽量简洁")
        self.agent.handle("/config-candidate-confirm 1")

        empty_preview = self.agent.handle("/preference-preview")
        self.agent.handle("/preference-enable 1")
        preview = self.agent.handle("/preference-preview 帮我总结知识库")
        help_text = self.agent.handle("/help")
        status = self.agent.handle("/status")
        manager_status = self.agent.handle("/config-manager-status")
        ...
        self.assertIn("/preference-preview", manager_status)

    def test_preference_apply_draft_command_reports_confirmation_boundary(self):
        self.agent.handle("/config-candidate-add preference 回答尽量简洁")
        self.agent.handle("/config-candidate-confirm 1")

        empty_draft = self.agent.handle("/preference-apply-draft 帮我总结知识库")
        self.agent.handle("/preference-enable 1")
        draft = self.agent.handle("/preference-apply-draft 帮我总结知识库")
        help_text = self.agent.handle("/help")
        status = self.agent.handle("/status")
        manager_status = self.agent.handle("/config-manager-status")
        preference_id = json.loads((self.paths.config_dir / "preferences.local.json").read_text(encoding="utf-8"))[
        ...
        self.assertIn("暂无已启用偏好", empty_draft)
        self.assertIn("待确认偏好应用草稿", draft)
        self.assertIn("确认状态：待用户显式确认", draft)
        self.assertIn("预览输入：帮我总结知识库", draft)
        self.assertIn(f"1. [{preference_id}] 回答尽量简洁", draft)
        self.assertIn("当前阶段不真正应用偏好", draft)
        self.assertIn("/preference-apply-draft [输入文本]：生成待确认偏好应用草稿", help_text)
    ...
    def test_inner_brain_eval_command_includes_local_evaluation_cases_without_training(self):
        ...
        self.assertIn("执行 InnerBrain 固定与本机评估集", log_content)
        old_log_message = "执行 InnerBrain " + "本地评估集"
        self.assertNotIn(old_log_message, log_content)
    ...
    def test_inner_brain_eval_failed_command_lists_only_failed_cases(self):
        ...
        self.assertIn("失败修复建议：", response)
        log_content = (self.paths.logs_dir / "jarvis.log").read_text(encoding="utf-8")
        self.assertIn("执行 InnerBrain 固定与本机评估集并只显示失败样本", log_content)
        old_log_message = "执行 InnerBrain " + "本地评估集并只显示失败样本"
        self.assertNotIn(old_log_message, log_content)
        self.assertFalse((self.paths.data_dir / "inner-brain" / "training" / "runtime.jsonl").exists())
    ...
    def test_inner_brain_eval_local_command_lists_only_local_cases(self):
        ...
        passed_file_line = (
            ...
            "总览：/inner-brain-eval-local-file aaa-real-log.jsonl"
        )
    ...
    def test_inner_brain_eval_local_command_guides_empty_local_evaluation_sample_target(self):
        response = self.agent.handle("/inner-brain-eval-local")
        ...
        self.assertIn("提示：补样本命令默认写入 runtime.jsonl。", response)
        self.assertNotIn("后续处理：", response)
        self.assertFalse((self.paths.data_dir / "inner-brain" / "training" / "runtime.jsonl").exists())
    ...
    def test_inner_brain_eval_local_failed_command_guides_empty_local_evaluation_samples(self):
        ...
        self.assertIn("说明：这些命令只写入本机 evaluation 样本，不自动训练。", response)
        self.assertIn("提示：补样本命令默认写入 runtime.jsonl。", response)
        self.assertNotIn("查看已处理样本", response)
        self.assertNotIn("按文件聚焦样本", response)
    ...
    def test_inner_brain_eval_local_failed_command_lists_only_failed_local_cases(self):
        ...
        self.assertIn("失败修复建议：", response)
        self.assertIn("后续处理：", response)
        self.assertIn("- 按文件聚焦待处理失败：/inner-brain-eval-local-file-failed 文件名", response)
        self.assertNotIn("- 按文件聚焦失败：/inner-brain-eval-local-file-failed 文件名", response)
    ...
    def test_inner_brain_eval_local_file_command_suggests_report_for_failed_selected_file(self):
        ...
        self.assertIn("FAIL 请看看资料库状态 -> knowledge.status", response)
        self.assertIn("后续处理：", response)
        self.assertIn("- 查看当前文件待处理失败样本：/inner-brain-eval-local-file-failed failed-log.jsonl", response)
    ...
    def test_inner_brain_eval_local_file_failed_command_lists_only_selected_file_failures(self):
        ...
        self.assertIn("失败修复建议：", response)
        self.assertIn("后续处理：", response)
        self.assertIn("- 当前文件总览：/inner-brain-eval-local-file failed-log.jsonl", response)
        self.assertIn("- 查看当前文件已处理样本：/inner-brain-eval-local-resolved failed-log.jsonl", response)
    ...
    def test_inner_brain_eval_local_resolved_command_lists_only_passed_local_cases(self):
        ...
        (evaluation_dir / "clean-log.jsonl").write_text(
            json.dumps(
                {
                    "text": "你好",
                    "expected_intent": "assistant.greeting",
        ...
        self.assertNotIn("FAIL 请看看资料库状态", response)
        self.assertIn("后续处理：", response)
        self.assertIn("- 查看待处理失败样本：/inner-brain-eval-local-failed", response)
        self.assertIn("- 按文件查看已处理样本：/inner-brain-eval-local-resolved 文件名", response)
        self.assertIn("可查看文件：", response)
        self.assertIn(
        ...
        self.assertIn(
            "- clean-log.jsonl：已处理 2 条，待处理失败 0 条："
            "总览：/inner-brain-eval-local-file clean-log.jsonl；"
            "已处理：/inner-brain-eval-local-resolved clean-log.jsonl",
            response,
        )
        self.assertNotIn(
        ...
        self.assertNotIn("failed-log.jsonl：", response)
        self.assertFalse((self.paths.data_dir / "inner-brain" / "training" / "runtime.jsonl").exists())

    def test_inner_brain_eval_local_resolved_command_can_filter_local_file(self):
        evaluation_dir = self.paths.data_dir / "inner-brain" / "evaluation"
        evaluation_dir.mkdir(parents=True)
        (evaluation_dir / "real-log.jsonl").write_text(
            json.dumps(
        ...
        self.assertIn("- 当前文件总览：/inner-brain-eval-local-file real-log.jsonl", response)
        self.assertIn("- 查看当前文件待处理失败样本：/inner-brain-eval-local-file-failed real-log.jsonl", response)
    ...
    def test_inner_brain_eval_local_resolved_command_suggests_report_for_file_with_pending_failures(self):
        ...
        self.assertIn("- 当前文件总览：/inner-brain-eval-local-file real-log.jsonl", response)
        self.assertIn("- 查看当前文件待处理失败样本：/inner-brain-eval-local-file-failed real-log.jsonl", response)
        self.assertIn("- 导出当前文件待处理失败报告：/inner-brain-eval-local-report real-log.jsonl", response)
        self.assertIn("- 查看全部已处理样本：/inner-brain-eval-local-resolved", response)
        self.assertFalse((self.paths.data_dir / "inner-brain" / "training" / "runtime.jsonl").exists())
    ...
    def test_inner_brain_eval_local_report_command_exports_failed_markdown_without_training(self):
        ...
        self.assertIn("待处理失败样本：1", response)
        self.assertIn("后续处理：", response)
        self.assertIn("- 查看待处理失败样本：/inner-brain-eval-local-failed", response)
        self.assertIn("- 按文件聚焦待处理失败：/inner-brain-eval-local-file-failed 文件名", response)
        self.assertNotIn("- 按文件聚焦失败：/inner-brain-eval-local-file-failed 文件名", response)
    ...
    def test_inner_brain_eval_add_command_saves_local_evaluation_case_without_training(self):
        ...
        self.assertIn("- 导出样本文件待处理失败报告：/inner-brain-eval-local-report runtime.jsonl", response)
    ...
    def test_task_failure_replay_includes_auto_captured_command_context(self):
        ...
        self.assertIn("样本建议：/inner-brain-eval-add /dir-add 工作区", failure_response)
        self.assertIn("=> /dir-add", failure_response)
