...
截至 2026-05-27，第一版已经落地 provider-neutral 的 `LLMRouter` / `LLMProvider` / `LLMIntent` 分层，并具备 `off`、`fake`、OpenAI Responses API provider、OpenAI-compatible Responses 端点、token usage 日志、`/llm-status` 配置诊断和 `/llm-usage` 本地汇总。Gemini、Qwen provider 后续应作为 adapter 接入，不让 `JarvisAgent` 直接感知厂商细节。
...
- 增加 fake provider 测试路径、OpenAI Responses API adapter、OpenAI-compatible 端点、token usage 日志、`/llm-status` 配置诊断和 `/llm-usage` 本地汇总。
