...
- 自动记忆与配置管家第一阶段：`/config-manager-status` 和 `/memory-config-status` 只读汇总长期记忆、经验记忆、常用目录、应用本地覆盖、LLM 本地配置、联网搜索本地配置和运行态候选池，并提示现有管理入口；`/config-candidate-add 类型 内容`、`/config-candidates` 和 `/config-candidate-dismiss 编号` 可显式记录、查看和忽略记忆与配置候选，本阶段不自动写入长期配置，API key 只显示已配置/未配置。
