...
> `0.148.0` 追加完成偏好应用状态输出面过滤第一阶段：`/preference-apply-status [编号或ID] [输出面]` 可只读过滤 `reply`、`knowledge` 或 `memory` 三类输出面，未知输出面仅返回提示，不改变确认、撤销、偏好启停、LLM 白名单、SearchRouter、InnerBrain、路由或桌面执行决策。

后续目标：

- 从 v108 起，下一阶段主线切换为 Jarvis Lite 1.0 验收路线。InnerBrain 本机 evaluation、LLM 外脑、联网搜索和桌面面板继续作为支撑能力维护，不再把报告文案、空状态和局部提示微调作为主线。
- 继续评估已确认偏好的按输出面撤销、按单条偏好解释或按单条偏好撤销；默认仍不从普通聊天或 LLM 白名单自动写入、启停或确认偏好。
