...
> InnerBrain 样本分类器优先、本机 evaluation 样本与报告闭环、LLM 外脑、Agent 联网搜索、桌面配置面板、桌面 smoke 清理，以及 1.0 路线第一步应用注册表第一阶段、只读窗口感知第一阶段和屏幕截图保存第一阶段已完成。当前桌面观察能力可读窗口和保存截图，但仍不启动、不切换、不点击、不输入。

后续目标：

- 从 v108 起，下一阶段主线切换为 Jarvis Lite 1.0 验收路线。InnerBrain 本机 evaluation、LLM 外脑、联网搜索和桌面面板继续作为支撑能力维护，不再把报告文案、空状态和局部提示微调作为主线。
- 继续实现 OCR、键鼠自动化基础、意图授权层、自动记忆与配置管家、五个常用应用首批工作流和失败复盘。
