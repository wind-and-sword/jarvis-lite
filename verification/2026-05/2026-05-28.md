...
## 2026-05-28 v6 高频别名迁移与 0.1.6 安装包验证

### 验证范围

- 版本提升到 `0.1.6`，把高频 legacy 别名迁移打入新的 Windows 可安装测试包。
- 验证覆盖安装脚本仍会关闭运行中的 `JarvisLite.exe`、覆盖程序文件、保留用户数据目录，并在弹窗中显示当前版本。
...
- 目标测试按预期失败，`pyproject.toml` 仍为 `0.1.5`，期望 `0.1.6`。

GREEN 结果：

- 版本一致性测试通过。
- `pyproject.toml`、`src/jarvis_lite/__init__.py`、`tests/test_project_metadata.py` 和 README 当前测试包说明已同步到 `0.1.6`。
...
- 代表句复扫：通过，输出 `legacy=0 unknown=0`。首次复扫脚本因引用不存在的 `jarvis_lite.paths` 模块失败；按项目实际 `build_project_paths` 修正后复跑通过。
...
## 2026-05-28 v6 高频 legacy 别名迁移验证

### 验证范围

- 问候、身份/能力、上下文、知识库、最近文件、日报、更新和经验状态的高频同义表达从 `legacy_fallback` 迁移到 `seed_sample`。
- `请帮我导入第二份最近文件到资料库` 归一为 `recent_file.import_numbered`。
...
- 高频别名测试 29 个子用例先失败，失败原因为 `source=legacy_fallback`。
- 礼貌前缀编号最近文件导入先失败，返回 `legacy.import_numbered_recent_file`。

GREEN 结果：

- 高频别名测试通过，29 个代表表达均返回 `source=seed_sample`。
- 编号最近文件导入目标测试通过，`请帮我导入第二份最近文件到资料库` 返回 `recent_file.import_numbered` 和 `result_index=2`。
...
- 覆盖问候、身份、能力、上下文、批量标签历史、知识库、最近文件、日报、更新、经验、编号最近文件、编号结果/建议和目录表达。
- 输出 `legacy=0 unknown=0`。

## 2026-05-28 v6 搜索后续动作与 0.1.5 安装包验证

### 验证范围

- 最近联网搜索结果支持自然语言按编号查看来源 URL：`打开第一条联网搜索结果` -> `/search-open 1`。
- 最近联网搜索结果支持 LLM 来源比较：`比较一下这些联网来源` -> `/search-compare`。
- 最近联网搜索摘要支持保存到 `word/` 和导入到 `data/` 知识库：`/search-save-summary`、`/search-import-summary`。
- 桌面快捷方式删除新增宾语前置表达：`把桌面快捷方式比特浏览器删掉`。
- InnerBrain 缺失槽位澄清输出补全命令、`/inner-brain-label` 和 `/inner-brain-teach` 提示。
- 版本提升到 `0.1.5` 并生成 Windows 安装包。
...
- 搜索后续动作先失败：`打开第一条联网搜索结果` 被误识别为目录打开，`/search-save-summary` 是未知命令，`导入这个搜索摘要到知识库` 被旧导入路径误吞。
- 桌面快捷方式宾语前置表达先失败，并落到 LLM fallback。
- InnerBrain 缺槽澄清先失败，只展示意图和缺失字段。
- 版本一致性测试先失败，`pyproject.toml` 仍为 `0.1.4`。

GREEN 结果：

- 搜索后续 5 项目标测试通过。
- 桌面快捷方式宾语前置表达 2 项目标测试通过。
- InnerBrain 澄清提示目标测试通过。
- 版本一致性目标测试通过。
...
- 源码桌面 smoke：通过，输出 `Jarvis Lite 桌面助手` 和 `desktopPetWindow`。
- 打包：生成 `E:\oyzj\ai\jarvis-lite-dist\JarvisLiteSetup.exe`，并复制 `E:\oyzj\ai\jarvis-lite-dist\JarvisLiteSetup-0.1.5.exe`，两个文件均为 57,065,472 bytes。
- 打包 exe smoke：通过，输出 `Jarvis Lite 桌面助手` 和 `desktopPetWindow`。
- `install.cmd`：包含 `taskkill`、`DisplayVersion /d "0.1.5"`、`Jarvis Lite 0.1.5 installed`、覆盖安装提示和用户数据保留提示。
- `JarvisLiteSetup.sed`：包含 `Jarvis Lite 0.1.5 installation finished`、`TargetName=E:\oyzj\ai\jarvis-lite-dist\JarvisLiteSetup.exe` 和 `AppLaunched=install.cmd`。
- `/inner-brain-preview 打开第一条联网搜索结果`：通过，输出 `意图：web_search.open_numbered`、`来源：seed_sample`、`命令：/search-open 1`、`编号：1`。
- `/inner-brain-preview 把桌面快捷方式比特浏览器删掉`：通过，输出 `意图：desktop.delete_shortcut`、`来源：seed_sample` 和 `对象：比特浏览器`。
- `git diff --check`：退出 0，仅提示 LF/CRLF 工作区换行警告。
- Markdown 本地链接检查：通过，输出 `Markdown local links OK`。
- 敏感信息差异扫描：通过，输出 `Sensitive diff scan OK`。
- `git ls-files config/llm.local.json config/search.local.json` 未输出 tracked 文件。
...
- `给 note.txt 打标签 项目` 不再依赖 `legacy_fallback` 主识别。
- `把 data/note.txt 标记为 项目 Python` 抽取显式文件路径和多个标签。
- Agent 执行层继续复用 `/tag`。

### RED：显式文件名标签仍走 legacy fallback
...
- 目标测试按预期失败，两个样例均返回 `command`，来源为 `legacy_fallback`。
...
- 3 项通过，结果 `OK`。
- `给 note.txt 打标签 项目` 返回 `document.tag_path`、`source=seed_sample`、`path=note.txt` 和 `tags=("项目",)`。
- `把 data/note.txt 标记为 项目 Python` 映射为 `/tag "note.txt" 项目 Python`。
