...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.136.0`，新增偏好应用预览第一阶段，`/preference-preview [输入文本]` 可只读预览已启用偏好的应用草案；当前阶段只展示预览，不改变回复风格、LLM prompt、路由或执行决策，不把偏好预览命令加入 LLM 白名单；完整进度见 `word/progress/2026-06-05.md`。
