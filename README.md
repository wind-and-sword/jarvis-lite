...
- 本地自然语言意图层，可用样本分类器处理问候、助手身份/能力询问、知识库、最近上下文、最近文件、读取编号资料、查看/导入编号最近文件、查看编号搜索结果、查看/执行编号建议、常用目录、日报、更新、经验、确认/取消执行、联网搜索和明确点名的桌面 `.lnk` 快捷方式删除；InnerBrain 高置信度命中时仍交给 `JarvisAgent` 执行。
