...
- 自动记忆与配置管家第一阶段：`/config-manager-status` 或 `/memory-config-status` 可只读汇总长期记忆、经验记忆、常用目录、应用本地覆盖、LLM 本地配置和联网搜索本地配置，并提示 `/remember`、`/experience`、`/dir-add`、`/apps`、`/llm-config-check` 和 `/search-config-check` 等管理入口；本阶段不自动写入长期配置，API key 只显示已配置/未配置。
...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.121.0`，新增任务失败截图 OCR 复盘第一阶段，可用 `/task-fail-capture 失败原因 [lang=chi_sim+eng]` 在显式失败记录时保存截图并 OCR，把截图路径、尺寸、OCR 文本或 OCR 失败诊断写入失败复盘；当前阶段不自动重新执行外部动作、不点击、不输入；完整进度见 `word/progress/2026-06-03.md`。
