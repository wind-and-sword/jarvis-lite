...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.129.0`，新增高风险记忆与配置候选确认草稿第一阶段，联系人别名、授权规则、应用别名和偏好候选执行固化时只生成确认草稿、撤销入口和不写入长期配置边界；当前阶段仍不自动写入高风险配置、不点击、不输入；完整进度见 `word/progress/2026-06-03.md`。
