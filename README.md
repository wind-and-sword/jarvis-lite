...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.122.0`，新增记忆与配置候选池第一阶段，可用 `/config-candidate-add 类型 内容` 显式记录运行态候选，再用 `/config-candidates` 查看、`/config-candidate-dismiss 编号` 忽略；当前阶段不自动写入长期记忆、配置、联系人或授权规则，也不把候选写入命令交给 LLM 白名单；完整进度见 `word/progress/2026-06-03.md`。
