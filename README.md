...
- 任务状态与失败复盘：`/task-status` 可查看当前任务、当前步骤、已完成步骤和最近失败记录，`/task-start 任务名称`、`/task-step 步骤说明`、`/task-fail 失败原因`、`/task-resume`、`/task-complete` 和 `/task-cancel` 可显式记录、恢复、完成或取消任务；`/task-fail-capture 失败原因 [lang=chi_sim+eng]` 可在显式失败记录时保存截图并 OCR，把截图路径、尺寸、OCR 文本或 OCR 失败诊断写入失败复盘；当前阶段不自动重新执行外部动作、不点击、不输入，不接入自然语言自动任务编排。
...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.123.0`，新增记忆与配置候选固化第一阶段，可用 `/config-candidate-apply 编号` 显式固化长期记忆、经验记忆或常用目录候选；联系人别名、授权规则、应用路径、偏好和其他候选仍不会自动写入长期配置，也不把候选固化命令交给 LLM 白名单；完整进度见 `word/progress/2026-06-03.md`。
