...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.124.0`，新增任务状态自动采集第一阶段，当前任务运行中执行普通命令会进入最近任务事件，失败复盘会带上自动采集上下文；当前阶段仍不自动重放外部动作、不点击、不输入；完整进度见 `word/progress/2026-06-03.md`。
