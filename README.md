...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.125.0`，新增任务执行结果摘要采集第一阶段，当前任务运行中执行显式命令后，最近任务事件会写回紧凑结果摘要，失败复盘会带上命令和结果；当前阶段仍不自动重放外部动作、不点击、不输入；完整进度见 `word/progress/2026-06-03.md`。
