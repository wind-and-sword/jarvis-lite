...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.114.0`，新增意图授权层第一阶段，可用 `/authorization-status` 查看执行决策策略，LLM 外脑桌面动作命令会降级说明且不自动执行；完整进度见 `word/progress/2026-06-03.md`。
