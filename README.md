...
- 桌面观察与基础键鼠能力：`/apps` 和 `/app-find` 可查看与匹配首批常用应用，`/app-launch 应用名称或别名` 可启动已登记且有可用路径的应用，`/chrome-workflow-status` 可查看 Chrome 工作流第一阶段边界，`/chrome-open URL` 可用 Chrome 打开明确网页，`/chrome-search 关键词` 可用 Chrome 打开搜索结果页，`/windows` 可只读查看当前 Windows 可见窗口和前台窗口，`/window-focus 编号或标题/应用名` 可切换到已存在的显式窗口，`/screenshot [文件名]` 可保存当前主屏幕 PNG，`/ocr-status` 和 `/ocr-image 图片路径 [lang=chi_sim+eng]` 可诊断 OCR 与识别指定图片，`/screen-ocr [文件名] [lang=chi_sim+eng]` 可截图后立即识别文字，`/hotkey key1+key2 [key3+key4 ...]` 可发送显式快捷键组合，`/mouse-click x y [button=left|right|middle]` 可执行显式坐标点击，`/type-text 文本` 可向当前焦点输入显式文本；当前阶段启动应用和 Chrome 打开网页后不自动切换窗口，不读取网页、不点击页面，不做目标识别或拖动，不把自然语言自动映射为键鼠执行。
...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.118.0`，新增 QQ/微信准备式工作流第一阶段，可用 `/messaging-workflow-status` 查看边界，用 `/qq-open`、`/wechat-open` 显式打开应用，用 `/qq-focus`、`/wechat-focus` 聚焦已有窗口，用 `/qq-prepare-message 联系人 => 消息` 或 `/wechat-prepare-message 联系人 => 消息` 生成未发送准备单；当前阶段不查找真实联系人、不点击、不输入、不发送消息；完整进度见 `word/progress/2026-06-03.md`。
...
真实 provider 的 instructions 会列出可返回的 Jarvis Lite 命令白名单，例如 `/kb-summary`、`/ask 问题`、`/read 文件名`、`/tag 文件名 标签...`、`/config-manager-status`、`/chrome-workflow-status`、`/search-summary 关键词`、`/search-open 编号`、`/search-compare`、`/search-save-summary [文件名]`、`/search-import-summary [文件名]`、`/llm-smoke [prompt]`、`/llm-config-init [provider]`、`/llm-config-set key=value ...`、`/llm-config-check`、`/search-smoke [query]`、`/search-config-init [provider]`、`/search-config-set key=value ...` 和 `/search-config-check`。模型不得返回列表之外的命令；参数不确定时应返回澄清问题。
