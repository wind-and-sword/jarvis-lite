...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.117.0`，新增 Clash Verge 低风险工作流第一阶段，可用 `/clash-workflow-status` 查看边界，用 `/clash-open` 显式打开代理面板，用 `/clash-focus` 聚焦已有代理面板窗口；当前阶段不切换节点、不开关系统代理、不修改配置；完整进度见 `word/progress/2026-06-03.md`。
