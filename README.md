...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.2.0`，包含 InnerBrain 样本分类器优先迁移、联网搜索入口、SearchRouter + LLMRouter 搜索总结组合流程、联网搜索后续来源处理、显式文件标签槽位迁移、高频 legacy 别名迁移、多轮澄清补槽闭环、联网搜索 query 补槽、编号+标签联合补槽、目录别名和经验内容补槽，以及多轮澄清 v1 收口：文件路径、编号资料、当前资料标签、标签组+新标签、经验搜索关键词和经验建议关键词可在下一句直接补齐。
