...
- InnerBrain 样本闭环：`/inner-brain-adopt 文本` 可采纳正确识别结果，`/inner-brain-label 文本 => intent [slot=value ...]` 可人工标注 unknown 或误识别样本，`/inner-brain-teach 文本 => /命令` 可把自然语言短句教学为已知命令；`/inner-brain-eval-add 文本 => /命令`、`/inner-brain-eval-label 文本 => intent [slot=value ...]`、`/inner-brain-eval-add-candidate 编号 => /命令` 和 `/inner-brain-eval-label-candidate 编号 => intent [slot=value ...]` 可先写入本机评估样本，不自动训练，保存后会提示复跑本机评估、只看失败、按 `runtime.jsonl` 聚焦和导出 `runtime.jsonl` 失败报告；本机 evaluation 为空时，`/inner-brain-eval-local` 和 `/inner-brain-eval-local-failed` 会提示这些添加入口；`/inner-brain-eval-local` 与 `/inner-brain-eval-local-file 文件名` 有样本时会提示只看失败、查看已处理和按文件聚焦入口，全量视图还会列出具体文件名候选，`/inner-brain-eval-local-file-failed 文件名` 可按本机 evaluation JSONL 文件排查失败样本并提示查看当前文件全部样本和当前文件已处理样本，`/inner-brain-eval-local-resolved [文件名]` 可只读查看当前已通过的本机 evaluation 样本，指定文件已处理视图会提示查看当前文件全部样本，全量已处理视图会列出有通过样本的具体文件名候选，并在候选行提示同文件总览和同文件已处理入口，对仍有失败的候选提示同文件失败入口，`/inner-brain-eval-local-failed` 会按失败来源文件汇总，并在失败文件分组提示同文件总览、同文件失败入口和按文件报告入口，`/inner-brain-eval-local-report [文件名]` 可把本机失败评估导出为 Markdown 报告并汇总失败类型、期望意图、意图混淆方向、文件意图混淆方向、失败原因、意图混淆修复建议和文件意图混淆修复建议，导出后会提示查看失败、按文件聚焦和补充 evaluation 样本，指定文件导出后还会提示查看当前文件全部样本和当前文件已处理样本；当已识别 intent 但缺少关键槽位时，支持下一句自然语言补齐导入路径或桌面快捷方式名称。
