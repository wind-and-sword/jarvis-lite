...
- InnerBrain 样本闭环：`/inner-brain-adopt 文本` 可采纳正确识别结果，`/inner-brain-label 文本 => intent [slot=value ...]` 可人工标注 unknown 或误识别样本，`/inner-brain-teach 文本 => /命令` 可把自然语言短句教学为已知命令；`/inner-brain-eval-add 文本 => /命令`、`/inner-brain-eval-label 文本 => intent [slot=value ...]`、`/inner-brain-eval-add-candidate 编号 => /命令` 和 `/inner-brain-eval-label-candidate 编号 => intent [slot=value ...]` 可先写入本机评估样本，不自动训练，保存后会提示复跑本机评估、只看待处理失败、按 `runtime.jsonl` 聚焦和导出 `runtime.jsonl` 待处理失败报告；本机 evaluation 为空时，`/inner-brain-eval-local` 和 `/inner-brain-eval-local-failed` 会提示这些添加入口；`/inner-brain-eval-local` 与 `/inner-brain-eval-local-file 文件名` 有样本时会提示只看失败、查看已处理和按文件聚焦入口，全量视图还会列出具体文件名候选，并在候选行把同文件总览标注为 `总览：/inner-brain-eval-local-file 文件名`，`/inner-brain-eval-local-file-failed 文件名` 可按本机 evaluation JSONL 文件排查失败样本并提示当前文件总览、当前文件已处理样本和全部待处理失败样本，`/inner-brain-eval-local-resolved [文件名]` 可只读查看当前已通过的本机 evaluation 样本，指定文件已处理视图会提示当前文件总览，全量已处理视图会列出有通过样本的具体文件名候选，并在候选行提示同文件总览和同文件已处理入口，对仍有失败的候选提示同文件失败入口，`/inner-brain-eval-local-failed` 会按失败来源文件汇总，并在失败文件分组提示同文件总览、同文件失败入口和按文件报告入口，`/inner-brain-eval-local-report [文件名]` 可导出本机评估待处理失败报告（Markdown）并汇总失败类型、期望意图、意图混淆方向、文件意图混淆方向、失败原因、意图混淆修复建议和文件意图混淆修复建议，导出反馈会显示待处理失败样本计数，并提示查看待处理失败、按文件聚焦待处理失败和补充 evaluation 样本，指定文件导出后还会提示当前文件总览、当前文件待处理失败、当前文件已处理样本和全部待处理失败样本；当已识别 intent 但缺少关键槽位时，支持下一句自然语言补齐导入路径或桌面快捷方式名称。
...
`/inner-brain-status` 会展示 InnerBrain 样本分类器优先状态、`legacy_fallback` 迁移期兼容状态、seed/runtime 样本数量、置信度阈值和运行态训练目录。`/inner-brain-eval` 会执行固定 seed 评估集，并合并读取 `data/inner-brain/evaluation/*.jsonl` 本机评估样本，输出通过数、失败数、准确率、用户可读来源计数和逐条样例；若评估失败，会追加显式训练建议，例如 `/inner-brain-teach 请看看资料库状态 => /kb-summary` 或 `/inner-brain-label 文本 => intent`，但仍不写入训练样本或运行态上下文。`/inner-brain-eval-failed` 复用同一评估集和修复建议，只输出失败样例；无失败时显示 `失败样例：- 无`。`/inner-brain-eval-local` 只执行 `local_evaluation` 本机评估样本，有样本时会提示 `/inner-brain-eval-local-failed`、`/inner-brain-eval-local-resolved` 和 `/inner-brain-eval-local-file 文件名`，并按失败数优先列出可聚焦文件名候选及每个文件通过/失败数量，候选行会把同文件总览标注为 `总览：/inner-brain-eval-local-file 文件名`，对仍有失败的文件提示同文件失败入口和按文件导出待处理报告入口；`/inner-brain-eval-local-failed` 只显示本机待处理失败样本、按失败数优先排序的失败来源文件分组、失败类型汇总、失败期望意图汇总、失败意图混淆汇总、失败文件意图混淆汇总、失败原因汇总、失败意图混淆分组修复建议、失败文件意图混淆分组修复建议和平铺修复建议，失败文件分组会提示同文件总览、同文件待处理失败和同文件待处理报告入口；本机 evaluation 样本为空时，两者会提示 `/inner-brain-eval-add`、`/inner-brain-eval-label`、`/inner-brain-eval-add-candidate` 和 `/inner-brain-eval-label-candidate` 这些只写本机 evaluation、不自动训练的添加入口。`/inner-brain-eval-local-report [文件名]` 会把本机评估待处理失败报告导出到 `word/inner-brain-evaluation-report.md`，文件名可选并同样支持省略 `.jsonl`，报告会按失败类型、期望意图、意图混淆方向、文件意图混淆方向和失败原因统计数量并列出典型样本，同时按意图混淆方向、来源文件+意图混淆方向分组列出显式修复建议；导出反馈会显示待处理失败样本计数，并以全部待处理失败标签提示 `/inner-brain-eval-local-failed`，指定文件时以当前文件待处理失败标签提示 `/inner-brain-eval-local-file-failed 文件名`、以当前文件总览标签提示 `/inner-brain-eval-local-file 文件名` 和 `/inner-brain-eval-local-resolved 文件名`，并提示 `/inner-brain-eval-add 文本 => /命令` 和 `/inner-brain-eval-label 文本 => intent [slot=value ...]`；指定单个文件导出时会省略文件意图混淆交叉汇总和文件级修复建议分组，避免重复列出同一个文件名。`/inner-brain-eval-local-file 文件名` 有样本时会提示当前文件失败视图、当前文件已处理清单和返回全部本机评估，`/inner-brain-eval-local-file-failed 文件名` 会进一步按本机 JSONL 文件名过滤失败样本，并提示当前文件总览、当前文件已处理样本和全部待处理失败样本，`/inner-brain-eval-local-resolved [文件名]` 会只读列出当前已通过的本机 evaluation 样本，指定文件已处理视图会提示当前文件总览，并在全量已处理视图列出有通过样本的具体文件名候选和同文件待处理失败数量，且按待处理失败数优先排序，并为仍有失败的文件提示同文件失败入口和待处理报告入口，提示回到失败视图继续处理待处理样本。`/inner-brain-eval-add 文本 => /命令` 会把显式命令期望写入 `data/inner-brain/evaluation/runtime.jsonl`，`/inner-brain-eval-label 文本 => intent [slot=value ...]` 会把显式 intent 期望写入同一评估文件；`/inner-brain-eval-add-candidate 编号 => /命令` 和 `/inner-brain-eval-label-candidate 编号 => intent [slot=value ...]` 会复用 `/inner-brain-candidates` 的当前候选编号，把候选原文写入同一评估文件；这些入口都只保存评估样本，不执行命令、不训练，保存反馈会提示 `/inner-brain-eval-local`、`/inner-brain-eval-local-failed`、`/inner-brain-eval-local-file runtime.jsonl` 和 `/inner-brain-eval-local-report runtime.jsonl`。本机评估样本格式为 `{"text":"请看看资料库状态","expected_intent":"knowledge.status","expected_policy":"execute","expected_command":"/kb"}`。`/inner-brain-preview 文本` 会只预览该文本被识别出的策略、意图、来源、置信度、槽位和命令，不执行任何本地动作；例如预览桌面快捷方式删除表达时不会删除 `.lnk` 文件。`/inner-brain-adopt 文本` 会把当前可识别的非 `unknown` 结果保存到 `data/inner-brain/training/runtime.jsonl`，重复样本不重复写入。`/inner-brain-label 文本 => intent [slot=value ...]` 用于人工标注 `unknown` 或误识别输入，例如 `command=/kb`、`items=比特浏览器`、`missing=source`。`/inner-brain-teach 文本 => /命令` 以及“以后我说“文本”就是 /命令”用于把短句绑定到已知命令，例如 `/kb`、`/kb-summary`、`/daily-report`；保存动作本身不会执行识别出的命令。
