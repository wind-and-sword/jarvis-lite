...
- 通过 `/kb-summary` 或“总结知识库”查看知识库资料摘要，摘要会先按标签分组，再列出每份资料的来源、可检索行数、标签和首行摘要预览；长预览会自动截断，摘要结果会写入最近资料列表，后续可继续说“读取第二份资料”或“给第一份资料打标签 项目”，也可以说“读取项目标签资料”先筛出某个标签下的资料组，或说“给项目标签资料都打标签 归档”生成批量打标签预览，再用“确认执行”或“取消执行”完成二段式处理，有标签时还会提示 `/ask 标签` 和按标签读取示例。
