...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.127.0`，新增任务失败复盘样本建议第一阶段，失败复盘可从显式命令事件生成可复制的 `/inner-brain-eval-add 原始输入 => /命令` 建议；当前阶段仍不自动写入 evaluation、不训练、不重放外部动作、不点击、不输入；完整进度见 `word/progress/2026-06-03.md`。
