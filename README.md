...
- 支持最近上下文：问答、读取资料、导入资料、查看最近文件、打开或整理目录、生成经验建议后，可继续说“查看最近上下文”“给这个资料打标签 项目”“读取这个资料”“读取第二份资料”“给第二份资料打标签 项目”“查看第一份最近文件”“导入第一份最近文件到知识库”“打开这个目录”“查看第一条建议”；“读取 note.md”“查看 note.txt”会复用 `/read` 并更新最近资料列表；最近上下文状态会列出最近资料列表、最近文件列表、最近建议、待确认建议命令、待确认批量打标签状态、最近批量打标签摘要和下一步建议；最近资料列表、最近文件列表、最近目录、最近搜索结果、最近建议和最近 5 条批量打标签历史会保存到项目外 `../jarvis-lite-runtime/agent-context.json`，可用 `/tag-history` 或“查看批量标签历史”查看，也可说“读取第一条标签历史资料”把历史影响资料恢复为最近资料列表；日报和最近上下文状态会引用这些上下文并生成“下一步建议”，其中最近文件建议会提示查看详情、导入知识库和刷新列表；同一交互或桌面会话内，执行建议采用“执行第 N 条建议”后再“确认执行”的两步确认，含参数草稿的建议也需要在同一会话内补全后确认。
- 读取 `data/` 目录中的 `.txt` 和 `.md` 文本资料，并通过 `/ask` 或普通问题返回最多 3 条带来源、命中摘要、命中原因和可继续操作的基础回答。
- 通过 `/kb` 查看个人知识库状态，包括资料文件数量、可检索行数和资料列表。
- 通过 `/kb-summary` 或“总结知识库”查看知识库资料摘要，摘要会先按标签分组，再列出每份资料的来源、可检索行数、标签和首行摘要预览；长预览会自动截断，摘要结果会写入最近资料列表，后续可继续说“读取第二份资料”或“给第一份资料打标签 项目”，也可以说“读取项目标签资料”先筛出某个标签下的资料组，或说“给项目标签资料都打标签 归档”生成批量打标签预览，再用“确认执行”或“取消执行”完成二段式处理；确认后会输出操作记录和逐份恢复提示，并写入批量打标签历史，有标签时还会提示 `/ask 标签` 和按标签读取示例。
