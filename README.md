...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.103.0`，包含 InnerBrain 样本分类器优先迁移、InnerBrain 包含签名置信度补偿、InnerBrain 固定评估集、本机评估 JSONL、失败修复建议、失败样本过滤视图、本机评估过滤视图、本机评估样本显式写入、候选编号写入本机评估样本、本机评估 JSONL 文件过滤、本机文件视图报告入口、本机失败评估按文件分组、本机失败视图失败文件分组当前文件总览入口、本机失败评估 Markdown 报告导出、本机失败评估原因汇总、本机失败评估类型汇总、本机失败评估期望意图汇总、本机失败评估意图混淆汇总、本机失败评估文件意图混淆汇总、本机失败评估意图混淆修复建议分组、本机失败评估文件意图混淆修复建议分组、本机评估空样本引导、本机评估样本保存后续验证提示、本机评估样本保存反馈待处理失败标签、本机文件候选待处理报告标签、本机报告导出待处理失败标题、本机报告处理边界待处理失败标签、本机失败视图文档待处理失败标签、本机报告指定文件样本计数提示、本机报告空筛选文件补样本写入提示、本机空评估视图补样本写入提示、本机已处理空视图行动提示、README 已处理空视图概要同步、PROJECT-PLAN 已处理空视图主干同步、本机失败帮助待处理标签、本机报告导出反馈待处理失败计数标签、本机全量反馈按文件聚焦待处理失败标签、全量评估运行日志固定与本机评估集标签、全量评估输出固定与本机评估集标签、全量评估帮助固定与本机评估集标签、本机 evaluation 保存反馈按文件报告入口、本机失败报告导出后续处理提示、本机失败报告导出反馈全量待处理失败标签、本机文件失败视图全部待处理失败报告标签、本机失败视图待处理失败报告标签、本机失败视图按文件待处理失败报告标签、本机当前文件待处理失败报告标签、本机失败报告导出反馈当前文件已处理入口、本机失败报告导出反馈当前文件总览入口、本机失败报告导出反馈当前文件总览标签、本机失败报告导出反馈当前文件待处理失败标签、本机当前文件失败反馈全部待处理失败标签、本机失败视图导出报告提示、本机失败视图文件聚焦提示、本机文件失败视图当前文件总览入口、本机文件失败视图当前文件总览标签、本机已处理指定文件视图当前文件总览入口、本机已处理指定文件视图当前文件总览标签、本机 evaluation 已处理样本只读清单、本机评估全量视图后续处理提示、本机评估全量视图文件名候选提示、本机评估全量视图文件候选状态摘要、本机评估全量视图文件候选失败优先排序、本机评估全量视图文件候选待处理入口、本机评估全量视图文件候选报告入口、本机评估全量视图文件候选总览入口、本机失败视图失败文件汇总排序、本机失败视图失败文件分组聚焦入口、本机失败视图失败文件分组报告入口、本机文件失败视图已处理入口、本机已处理视图文件候选提示、本机已处理视图文件候选状态摘要、本机已处理视图文件候选待处理优先排序、本机已处理视图文件候选待处理入口、本机已处理视图文件候选报告入口、本机已处理视图文件候选当前文件总览入口、本机已处理指定文件视图报告入口、联网搜索入口、SearchRouter + LLMRouter 搜索总结组合流程、联网搜索后续来源处理、多轮澄清 v1 收口、外脑 provider 配置闭环 v1、运行态配置初始化 v1、本地配置检查 v1、本地配置写入 v1、连通性诊断 v1、桌面配置面板 v1、LLM 外脑多轮澄清 v1、LLM 澄清状态持久化 v1、LLM 澄清轮数与过期策略 v1、桌面外脑待补充状态 v1、桌面外脑运行状态 v1、最近路由决策状态 v1、路由决策解释详情 v1、最近路由历史 v1、路由历史详情 v1、InnerBrain 训练候选 v1、候选按编号教学 v1、候选按编号标注 v1、候选频次排序 v1、候选运行态统计 v1、桌面内脑候选快捷入口 v1、桌面候选训练模板填充 v1、桌面候选模板状态同步 v1、桌面候选选择绑定 v1、桌面候选目标预填 v1，以及桌面 smoke 退出清理：桌面面板会固定展示当前 LLM 待补充问题、澄清轮次、外脑启用状态、最近一次 LLM 调用结果、最近一条用户输入的处理路径、该路径的选择依据和最近 5 条路由历史；聊天区可用 `/route-history` 查看最近 5 条路由的完整输入、结果和依据，也可用 `/inner-brain-eval` 执行固定评估集和本机评估样本，并在失败时得到 `/inner-brain-teach` 或 `/inner-brain-label` 显式修复建议；评估样本较多时可用 `/inner-brain-eval-failed` 只看失败样本和修复建议；只想查看本机 `data/inner-brain/evaluation/*.jsonl` 样本时可用 `/inner-brain-eval-local`，本机样本为空时会显示只写 evaluation、不自动训练的添加入口，并提示补样本命令默认写入 `runtime.jsonl`，有样本时会提示只看待处理失败、查看已处理样本和按文件聚焦样本，并按失败数优先列出可复制的具体文件名聚焦命令及每个文件的通过/失败数量，候选行会把同文件总览标注为 `总览：/inner-brain-eval-local-file 当前文件名`，对仍有待处理失败的文件提示同文件待处理失败入口和按文件导出待处理报告入口；本机待处理失败项可用 `/inner-brain-eval-local-failed`，该失败视图会按失败数优先显示失败文件分组，并在每个失败文件行提示同文件总览、同文件待处理失败入口和按文件导出待处理报告入口，有失败样本时会提示按文件聚焦待处理失败、返回全部待处理失败样本、导出待处理失败报告、按文件导出待处理失败报告、导出全部待处理失败报告以及导出当前文件待处理失败报告，并可用 `/inner-brain-eval-local-report [文件名]` 导出本机评估待处理失败报告到 `word/inner-brain-evaluation-report.md`，报告会按失败类型、期望意图、意图混淆方向、文件意图混淆方向和失败原因统计数量并列出典型样本，同时按意图混淆方向、来源文件+意图混淆方向分组列出显式修复建议，报告导出反馈会显示待处理失败样本计数，指定文件报告还会显示当前文件样本数，当前筛选文件无样本时会提示补样本命令默认写入 `runtime.jsonl`，并提示继续查看待处理失败样本、按文件聚焦待处理失败和补样本，指定文件报告还会提示当前文件总览、当前文件待处理失败、当前文件已处理样本和全部待处理失败样本；要聚焦某个 JSONL 文件时可用 `/inner-brain-eval-local-file 文件名` 或 `/inner-brain-eval-local-file-failed 文件名`，当前文件总览若仍有待处理失败会提示导出当前文件待处理失败报告，其中当前文件待处理失败视图会以 `当前文件总览：/inner-brain-eval-local-file 当前文件名` 提示回到总览，并提示 `/inner-brain-eval-local-resolved 当前文件名` 对照已处理样本、查看全部待处理失败样本和导出全部待处理失败报告，也可用 `/inner-brain-eval-local-resolved [文件名]` 只读查看当前已通过的本机 evaluation 样本，暂无已处理样本时会提示这里只显示已通过样本并引导查看待处理失败或补充本机 evaluation 样本，指定文件已处理视图会以 `当前文件总览：/inner-brain-eval-local-file 当前文件名` 提示回到总览，若仍有待处理失败也会提示导出当前文件待处理失败报告，全量已处理视图会列出有通过样本的具体文件名候选，显示同文件待处理失败数量，并按待处理失败数优先排序，候选行提示同文件总览和同文件已处理入口，对仍有待处理失败的文件提示同文件待处理失败入口和按文件导出待处理报告入口；想先沉淀真实日志而不训练时可用 `/inner-brain-eval-add 文本 => /命令` 或 `/inner-brain-eval-label 文本 => intent [slot=value ...]` 写入本机评估样本，保存反馈会提示复跑本机评估、只看待处理失败样本、聚焦 `runtime.jsonl` 和按 `runtime.jsonl` 导出待处理失败报告；用 `/inner-brain-candidates` 查看适合人工教学的内脑训练候选，候选出现次数会写入本地运行态统计，跨最近 5 条路由历史继续保留；桌面面板和托盘可直接点击“内脑候选”查看候选；桌面面板可选择候选编号或直接从候选下拉框选择单条候选，并用常见命令或常见 `intent slot=value` 模板预填 teach/label 训练目标，候选列表为空时模板按钮和候选下拉框会禁用，有候选时编号上限会同步到实际候选数；再用 `/inner-brain-teach-candidate 编号 => /命令` 显式教学，或用 `/inner-brain-label-candidate 编号 => intent slot=value` 显式标注；想先评估候选而不训练时，可用 `/inner-brain-eval-add-candidate 编号 => /命令` 或 `/inner-brain-eval-label-candidate 编号 => intent slot=value` 写入本机评估样本，保存后候选仍保留。
