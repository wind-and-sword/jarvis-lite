...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.119.0`，新增 IDEA 项目状态第一阶段，可用 `/idea-workflow-status` 查看边界，用 `/idea-open`、`/idea-focus`、`/idea-open-project 项目路径` 显式打开 IDEA、聚焦已有 IDEA 窗口或用 IDEA 打开项目目录，用 `/idea-project-status [项目路径]` 只读检查项目状态；当前阶段不运行测试、不打开终端、不点击、不输入、不编辑项目文件；完整进度见 `word/progress/2026-06-03.md`。
