...
- InnerBrain 本地内脑第一版：把既有自然语言规则包装为 `legacy_rule`，并用内置 seed 样本和 `data/inner-brain/training/*.jsonl` 运行态样本做轻量相似度识别，输出 `intent`、`slots`、`confidence`、`missing`、`source`、`reason` 和执行策略；可用 `/inner-brain-adopt 文本` 将正确识别结果沉淀为 runtime 样本。
...
`/inner-brain-status` 会展示 InnerBrain 是否启用 legacy 规则、seed/runtime 样本数量、置信度阈值和运行态训练目录。`/inner-brain-preview 文本` 会只预览该文本被识别出的策略、意图、来源、置信度、槽位和命令，不执行任何本地动作；例如预览桌面快捷方式删除表达时不会删除 `.lnk` 文件。`/inner-brain-adopt 文本` 会把当前可识别的非 `unknown` 结果保存到 `data/inner-brain/training/runtime.jsonl`，重复样本不重复写入，保存动作本身也不会执行识别出的命令。
