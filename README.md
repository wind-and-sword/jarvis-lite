...
- InnerBrain 样本闭环：`/inner-brain-adopt 文本` 可采纳正确识别结果，`/inner-brain-label 文本 => intent [slot=value ...]` 可人工标注 unknown 或误识别样本，`/inner-brain-teach 文本 => /命令` 可把自然语言短句教学为已知命令；`/inner-brain-eval-add 文本 => /命令`、`/inner-brain-eval-label 文本 => intent [slot=value ...]`、`/inner-brain-eval-add-candidate 编号 => /命令` 和 `/inner-brain-eval-label-candidate 编号 => intent [slot=value ...]` 可先写入本机评估样本，不自动训练；`/inner-brain-eval-local-file 文件名` 与 `/inner-brain-eval-local-file-failed 文件名` 可按本机 evaluation JSONL 文件排查样本，`/inner-brain-eval-local-failed` 会按失败来源文件汇总；当已识别 intent 但缺少关键槽位时，支持下一句自然语言补齐导入路径或桌面快捷方式名称。
