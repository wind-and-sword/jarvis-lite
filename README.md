...
- 桌面观察能力：`/apps` 和 `/app-find` 可只读查看与匹配首批常用应用，`/windows` 可只读查看当前 Windows 可见窗口和前台窗口，`/screenshot [文件名]` 可保存当前主屏幕 PNG，`/ocr-status` 和 `/ocr-image 图片路径 [lang=chi_sim+eng]` 可诊断 OCR 与识别指定图片，`/screen-ocr [文件名] [lang=chi_sim+eng]` 可截图后立即识别文字；当前阶段仍不启动、不切换、不点击、不输入。
...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.108.0`，新增截图 OCR 串联第一阶段，可用 `/screen-ocr [文件名] [lang=chi_sim+eng]` 保存当前主屏幕截图并立即识别文字；完整进度见 `word/progress/2026-06-03.md`。
