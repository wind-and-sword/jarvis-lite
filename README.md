...
- 支持最近上下文：问答、导入资料、打开或整理目录、生成经验建议后，可继续说“查看最近上下文”“给这个资料打标签 项目”“打开这个目录”“查看第一条建议”；最近上下文状态会列出最近建议和待确认建议命令；最近资料、最近目录、最近搜索结果和最近建议会保存到项目外 `../jarvis-lite-runtime/agent-context.json`，日报会引用这些最近上下文并生成“下一步建议”；同一交互或桌面会话内，执行建议采用“执行第 N 条建议”后再“确认执行”的两步确认，含参数草稿的建议也需要在同一会话内补全后确认。
