...
- 自动记忆与配置管家：`/config-manager-status` 或 `/memory-config-status` 可只读汇总长期记忆、经验记忆、常用目录、联系人别名、授权规则、应用本地覆盖、LLM 本地配置、联网搜索本地配置和运行态候选池，并提示 `/remember`、`/experience`、`/dir-add`、`/apps`、`/authorization-status`、`/config-candidates`、`/llm-config-check` 和 `/search-config-check` 等管理入口；`/config-candidate-add 类型 内容`、`/config-candidates`、`/config-candidate-history`、`/config-candidate-apply 编号`、`/config-candidate-confirm 编号`、`/config-candidate-undo 编号`、`/config-candidate-restore 编号` 和 `/config-candidate-dismiss 编号` 可显式记录、查看、查看历史、固化、确认、撤销、恢复和忽略记忆与配置候选；固化第一阶段只直接写入长期记忆、经验记忆和常用目录，联系人别名候选需要显式确认后写入 `config/contacts.local.json`，应用别名候选需要显式确认后写入 `config/apps.local.json` 的 `aliases`，授权规则候选需要显式确认后写入 `config/authorization.local.json`，撤销固化会删除对应别名或规则并恢复候选为活跃；偏好仍只生成确认草稿或暂不支持真实确认，不自动写入长期配置，API key 只显示已配置/未配置。
...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.133.0`，新增授权规则确认固化与撤销第一阶段，`/config-candidate-confirm 编号` 可把授权规则候选写入 `config/authorization.local.json`，`/config-candidate-undo 编号` 可删除对应授权规则并恢复候选；当前阶段只持久化和展示规则，不改变命令执行决策，不把确认/撤销命令加入 LLM 白名单；完整进度见 `word/progress/2026-06-05.md`。
