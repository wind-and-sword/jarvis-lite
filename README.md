...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.3.0`，包含 InnerBrain 样本分类器优先迁移、联网搜索入口、SearchRouter + LLMRouter 搜索总结组合流程、联网搜索后续来源处理、显式文件标签槽位迁移、高频 legacy 别名迁移、多轮澄清 v1 收口，以及外脑 provider 配置闭环 v1：`qwen`、`gemini` 可作为 provider alias 写入 `llm.local.json`，由 OpenAI-compatible adapter 执行，并在 `/llm-status`、`/llm-enable` 和配置模板中展示实际 adapter。
