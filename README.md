...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.135.0`，新增偏好显式启用与停用第一阶段，`/preference-status` 可查看已保存偏好启用状态，`/preference-enable 编号` 和 `/preference-disable 编号` 可显式启用或停用偏好；当前阶段只记录启用状态，不改变回复风格、LLM prompt、路由或执行决策，不把偏好管理命令加入 LLM 白名单；完整进度见 `word/progress/2026-06-05.md`。
