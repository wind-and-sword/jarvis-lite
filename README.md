...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.116.0`，新增 Chrome 低风险工作流第一阶段，可用 `/chrome-workflow-status` 查看边界，用 `/chrome-open URL` 或 `/chrome-search 关键词` 显式打开网页或搜索结果；完整进度见 `word/progress/2026-06-03.md`。
