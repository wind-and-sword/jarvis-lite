...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.140.0`，新增偏好应用确认记录与撤销第一阶段，`/preference-apply-confirm [输入文本]` 会在有已启用且无明显冲突偏好时确认偏好仅应用到本次显式命令输出并写入运行态确认记录，`/preference-apply-history` 可查看最近记录，`/preference-apply-undo 编号或ID` 只撤销确认记录；无启用偏好或存在明显冲突时拒绝确认且不写入记录；当前阶段不自动改变普通回复、LLM prompt、路由或执行决策，不把偏好管理命令加入 LLM 白名单；完整进度见 `word/progress/2026-06-09.md`。
