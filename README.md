...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.128.0`，新增记忆与配置候选恢复第一阶段，可查看已忽略/已固化候选并显式恢复为活跃候选；当前阶段仍不自动删除已写入长期存储、不自动写入高风险配置、不点击、不输入；完整进度见 `word/progress/2026-06-03.md`。
