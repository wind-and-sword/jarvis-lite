...
- 自动记忆与配置管家：`/config-manager-status` 或 `/memory-config-status` 可只读汇总长期记忆、经验记忆、常用目录、应用本地覆盖、LLM 本地配置、联网搜索本地配置和运行态候选池，并提示 `/remember`、`/experience`、`/dir-add`、`/apps`、`/config-candidates`、`/llm-config-check` 和 `/search-config-check` 等管理入口；`/config-candidate-add 类型 内容`、`/config-candidates`、`/config-candidate-history`、`/config-candidate-apply 编号`、`/config-candidate-restore 编号` 和 `/config-candidate-dismiss 编号` 可显式记录、查看、查看历史、固化、恢复和忽略记忆与配置候选；固化第一阶段只写入长期记忆、经验记忆和常用目录，恢复候选只改变运行态候选状态，不自动删除已经写入的长期存储，不自动写入联系人、授权规则、应用路径或其他长期配置，API key 只显示已配置/未配置。
...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.130.0`，新增任务失败复盘窗口与授权摘要第一阶段，`/task-fail` 和 `/task-fail-capture` 会在失败复盘中记录当前只读窗口摘要、路由摘要和授权摘要；当前阶段仍不切换窗口、不点击、不输入、不自动重新执行外部动作；完整进度见 `word/progress/2026-06-04.md`。
