...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.126.0`，新增任务失败复盘行动建议第一阶段，普通失败缺少屏幕/OCR 上下文时会提示可用 `/task-fail-capture 失败原因` 补充截图和 OCR；当前阶段仍不自动重放外部动作、不点击、不输入；完整进度见 `word/progress/2026-06-03.md`。
