...
- 桌面观察与基础键鼠能力：`/apps` 和 `/app-find` 可查看与匹配首批常用应用，`/app-launch 应用名称或别名` 可启动已登记且有可用路径的应用，`/windows` 可只读查看当前 Windows 可见窗口和前台窗口，`/window-focus 编号或标题/应用名` 可切换到已存在的显式窗口，`/screenshot [文件名]` 可保存当前主屏幕 PNG，`/ocr-status` 和 `/ocr-image 图片路径 [lang=chi_sim+eng]` 可诊断 OCR 与识别指定图片，`/screen-ocr [文件名] [lang=chi_sim+eng]` 可截图后立即识别文字，`/hotkey key1+key2 [key3+key4 ...]` 可发送显式快捷键组合，`/mouse-click x y [button=left|right|middle]` 可执行显式坐标点击，`/type-text 文本` 可向当前焦点输入显式文本；当前阶段启动应用后不自动切换窗口，不做目标识别或拖动，不把自然语言自动映射为键鼠执行。
...
- 安装完成弹窗会显示 `Jarvis Lite <版本号> installation finished`，用于确认新安装包已经生效；本轮测试包为 `0.115.0`，新增自动记忆与配置管家第一阶段，可用 `/config-manager-status` 只读查看记忆、目录、应用覆盖和 provider 配置状态；完整进度见 `word/progress/2026-06-03.md`。
