...
当前版本：`0.144.0`。当前方案入口见 [word/PROJECT-PLAN.md](word/PROJECT-PLAN.md)，正式文档索引见 [word/文档索引.md](word/文档索引.md)。
...
- 偏好应用链路：已保存偏好可显式启停、预览、生成草稿、确认、查看历史和撤销确认；最近有效确认记录可进入普通 LLM fallback 上下文和本地回答附注，但不进入 LLM 命令白名单、SearchRouter、InnerBrain、路由或桌面执行决策；`0.144.0` 起本地回答附注只对本地知识库回答和长期记忆兜底回答生效，并显示 `回答类型：本地知识库回答` 或 `回答类型：长期记忆兜底回答`。
...
`0.144.0` 测试包路径：`E:\ai\jarvis-lite-dist\JarvisLiteSetup-0.144.0.exe`。
...
- `/preference-status`、`/preference-preview [输入文本]`、`/preference-apply-confirm [输入文本]`、`/preference-apply-history`、`/preference-apply-undo 编号或ID`：偏好管理和确认审计。
...
- [word/progress/2026-06-09.md](word/progress/2026-06-09.md)：当日进度摘要。
- [verification.md](verification.md)：验证记录短入口。
- [verification/2026-06/2026-06-09.md](verification/2026-06/2026-06-09.md)：当日完整验证记录。
